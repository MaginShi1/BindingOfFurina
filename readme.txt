...
local RANGE_MULTIPLIER = 1.5          -- Multiply player's range for spawn offset
...
-- Additional tear slow constant
local EXTRA_TEAR_SLOW = 0.8
...
function mod:HandleUpdate(familiar)
    ...
    local spawnRange    = baseRange * RANGE_MULTIPLIER

    -- Apply extra tear slowdown
    shotSpeed = shotSpeed - EXTRA_TEAR_SLOW

    -- Clamp values
    tearDamage = math.max(tearDamage, 0)
    fireDelay  = math.max(fireDelay, 1)
    shotSpeed  = math.max(shotSpeed, 0.1)
    spawnRange = math.max(spawnRange, 0)
    ...
    if shooting then
        ...
        if familiar.FireCooldown <= 0 then
            ...
            if direction then
                data.idleCounter = 0
                -- Calculate spawn position offset by range
                local spawnPos = familiar.Position + direction * spawnRange
                local velocity = direction * (shotSpeed * BASE_TEAR_SPEED_MULTIPLIER)
                                  + player:GetTearMovementInheritance(direction)
                local tear = Isaac.Spawn(EntityType.ENTITY_TEAR, TearVariant.BLUE, 0,
                                          spawnPos, velocity, familiar):ToTear()
                tear.Scale = familiar.TearScale or 1
